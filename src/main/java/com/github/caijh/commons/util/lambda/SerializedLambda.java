...
import com.github.caijh.commons.util.ClassUtils;
...
public class SerializedLambda implements Serializable {
     ...
     * @throws SerializedLambdaResolveException if resolve fail
     */
    public static SerializedLambda resolve(SFunction<?, ?> lambda) {
        if (!lambda.getClass().isSynthetic()) {
            throw new SerializedLambdaResolveException("该方法仅能传入 lambda 表达式产生的合成类");
        }
        try (ObjectInputStream objIn = new ObjectInputStream(new ByteArrayInputStream(Objects.requireNonNull(SerializationUtils.serialize(lambda)))) {
            ...
            throw new SerializedLambdaResolveException("This is impossible to happen", e);
        }
    }
